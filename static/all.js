...
let my_icon = [
    ...
    new BMap.Icon('/static/img/0.png', new BMap.Size(50, 50), { offset: new BMap.Size(0, 0), imageOffset: new BMap.Size(5, 5) })
];

let old_did = [-1, -1, -1];
let marker = [undefined, undefined, undefined];
let colorcur = ['red', 'gray', 'blue'];
...
function draw(pid, did) {
    ...
    if (marker[pid] == undefined) {
        ...
        marker[pid] = new BMap.Marker(device_point[did], { icon: my_icon[pid] });
        map.addOverlay(marker[pid]);
        map.addOverlay(new BMapLib.CurveLine([device_point[old_did[pid]], device_point[did]], { strokeColor: colorcur[pid], strokeWeight: 3, strokeOpacity: 0.5 }));
        old_did[pid] = did;
    }
